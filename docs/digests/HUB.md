...
│   └── src/ packet · commands · client · spline · move · film · limits · timecode · export3d · simulator
│   └── test/ 134 tests: byte-exact vs firmware samples + e2e vs simulator + timecode/DF + export
...
- **NEXT (software):** event-track UI on the timeline + trigger backends — simulated first, then Arduino-class serial (Tier 1 **and** Tier 2), Enttec DMX and OSC (both need no new dependency); MIDI last, since every Node binding is native. Wire `ABORT` into `stopAll()` before any cue output drives something that moves. Then the export UI (format, units, up-axis, calibration, lens).
