...
│   └── test/ 214 tests: byte-exact vs firmware samples + e2e vs simulator + timecode/DF + export
...
│                              # lens lanes (focus/iris/zoom) + lens maps — authoring/export only
...
└── docs/ adr/ (why, 0000-0017) + digests/ (how, you are here)
...
19. **Nothing drives a lens motor.** ADR-0017 §4 specifies GRAFFIK-TRIG protocol v2 (device-side lens curve, one `GO`); it is not built. The app discloses this on the timeline band and on Upload — never let a UI imply a pull will happen.
...
- **NEXT (software):** build ADR-0017 §4 — GRAFFIK-TRIG v2 firmware + the host-side lens upload — which is what turns the lens lanes from authoring into motion. Then: a reusable **per-lens map library** (Preston stores 150; ours live in the move file), sustained-cue editing (drag the tail), cue duplication, MIDI last since every Node binding is native.
