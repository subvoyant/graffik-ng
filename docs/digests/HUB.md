...
│   └── src/ packet.ts | commands.ts | client.ts | spline.ts | move.ts | film.ts | limits.ts | simulator.ts
│   └── test/ 57 tests: byte-exact vs firmware samples + e2e vs simulator
...
│                              # soft limits · preferences (userData/preferences.json) · save/load · fw gate
...
└── docs/ adr/ (why, 0000-0013) + digests/ (how, you are here)
    + DEVELOPMENT.md (setup/daily-loop/troubleshooting)
    + HARDWARE-BRINGUP.md (first-contact + repeatability test protocol) + images/
```
...
14. UI uses design tokens only — **no literal hex in new UI** (ADR-0012). Axis colors are validated for colorblind separation; a new series color comes from the next validated categorical slot, never invented.
...
| [jog-slice.md](jog-slice.md) | Electron app, IPC channel table, prefs, limit enforcement, gamepad ballistics, editor interactions, packaging | `apps/jog-slice/*` @ 2026-08-17 (v0.6) |
